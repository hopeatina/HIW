...
from;
"./whitespace"; // Object type used to represent tokens. Note that normally, tokens
...
const;
pp = Parser.prototype; // Are we running under Rhino?
const;
isRhino = typeof Packages !== "undefined"; // Move to the next token
...
pp.skipBlockComment = function() {
    let;
    startLoc = this.options.onComment && this.options.locations && this.curPosition();
    let;
...
pp.finishOp = function(type, size) {
    let;
    str = this.input.slice(this.pos, this.pos + size);
    this.pos += size;
...
pp.readRegexp = function() {
    let;
    escaped, inClass, start = this.pos;
    for (;;) {
    ...
    let;
    value = null; // Rhino's regular expression parser is flaky and throws uncatchable exceptions,
...
pp.readInt = function(radix, len) {
    let;
    start = this.pos, total = 0;
    for (let i = 0, e = len == null ? Infinity : len; i < e;
    ++i);
    {
        let;
        code = this.input.charCodeAt(this.pos), val;
...
pp.readNumber = function(startsWithDot) {
    let;
    start = this.pos, isFloat = false, octal = this.input.charCodeAt(this.pos) === 48;
    ...
    if (isIdentifierStart(this.fullCharCodeAtPos())) this.raise(this.pos, "Identifier directly after number");
    let;
    str = this.input.slice(start, this.pos), val;
...
pp.readCodePoint = function() {
    let;
    ch = this.input.charCodeAt(this.pos), code;
    if (ch === 123) {
...
pp.readEscapedChar = function() {
    ...
    ch = this.input.charCodeAt(++this.pos);
    let;
    octal = /^[0-7]+/.exec(this.input.slice(this.pos, this.pos + 3));
...
pp.readWord1 = function() {
    containsEsc = false;
    let;
    word = "", first = true, chunkStart = this.pos;
    let;
